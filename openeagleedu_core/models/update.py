...
from eagle import models, api, release

API_ENDPOINT = "https://srv.openeagleedu.org/publisher-warranty/"
...
class PublisherWarrantyContract(models.Model):
    ...
    def _get_message(self):
        ...
        apps = self.env['ir.module.module'].sudo().search_read(domain,
                                                               ['name'])
        openeagleedu_instance_key = IrParamSudo.get_param(
            'database.openeagleedu_instance_key')
        openeagleedu_instance_hash_key = IrParamSudo.get_param(
            'database.openeagleedu_instance_hash_key')
        openeagleedu_hash_validate_date = IrParamSudo.get_param(
            'database.hash_validated_date')
        openeagleedu_expiration_date = IrParamSudo.get_param(
            'database.openeagleedu_expire_date')
        ...
        msg = {
            ...
            "openeagleedu_hash_validate_date": openeagleedu_hash_validate_date,
            "enterprise_code": str(openeagleedu_instance_key
                                   ) + "," + str(openeagleedu_instance_hash_key),
            "openeagleedu_expire_date": openeagleedu_expiration_date,
        }
